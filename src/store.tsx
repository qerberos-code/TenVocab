...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { (async () => { try { const raw = await AsyncStorage.getItem(KEY); if (raw) { const s = JSON.parse(raw); setStats(s.stats || emptyStats()); setStreak(s.streak ?? 7); } } finally { setLoading(false); } })(); }, []);
  useEffect(() => { if (!loading) AsyncStorage.setItem(KEY, JSON.stringify({ stats, streak })).catch(() => {}); }, [stats, streak, loading]);
  ...
  const recordKnowledge = (id: string, knew: boolean) => setStats(prev => { const s = prev[id] || { seen:0, correct:0, incorrect:0, mastery:0, nextReview:null }; const seen=s.seen+1, correct=s.correct+(knew?1:0), incorrect=s.incorrect+(knew?0:1); const mastery=Math.min(100, Math.round(((correct + 0.5) / (seen + 1)) * 100)); const delay=knew ? Math.min(14, 1+Math.floor(mastery/25)) : 0; return { ...prev, [id]: { seen, correct, incorrect, mastery, nextReview: Date.now()+delay*86400000 } }; });
  const recordQuiz = (id: string, correct: boolean) => { setScore(s=>s+(correct?1:0)); setAnswers(a=>[...a,id]); setStats(prev => { const s=prev[id] || { seen:0, correct:0, incorrect:0, mastery:0, nextReview:null }; const seen=s.seen+1, c=s.correct+(correct?1:0), i=s.incorrect+(correct?0:1); const mastery=Math.min(100, Math.round((c/(seen||1))*100)); const days = correct ? (mastery >= 85 ? 7 : mastery >= 65 ? 3 : 1) : 0; return { ...prev, [id]: { seen, correct:c, incorrect:i, mastery, nextReview: Date.now()+days*86400000 } }; }); setSessionIndex(i=>i+1); };
  const finishSession = () => { if (todayIds.length) setStreak(s=>s+1); setSessionIndex(0); };
  const resetDemo = () => { setStats(emptyStats()); setTodayIds([]); setSessionIndex(0); setScore(0); setAnswers([]); setStreak(7); AsyncStorage.removeItem(KEY).catch(()=>{}); };
