...
export const isMastered = (s?: { mastery: number }) => (s?.mastery ?? 0) >= MASTERED_AT;
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  const recordQuiz = (id: string, correct: boolean) => { if (todayIds.length && sessionIndex >= todayIds.length) return; setScore(s=>Math.min(todayIds.length||s+1, s+(correct?1:0))); setAnswers(a=>[...a,id]); setStats(prev => { const s=prev[id] || { seen:0, correct:0, incorrect:0, mastery:0, nextReview:null }; const seen=s.seen+1, c=s.correct+(correct?1:0), i=s.incorrect+(correct?0:1); const mastery=Math.min(100, Math.round(((c + 0.5)/(seen + 1))*100)); const days = correct ? (mastery >= MASTERED_AT ? 7 : mastery >= 65 ? 3 : 1) : 0; return { ...prev, [id]: { seen, correct:c, incorrect:i, mastery, nextReview: Date.now()+days*86400000 } }; }); setSessionIndex(i=>i+1); if (todayIds.length && sessionIndex >= todayIds.length-1) { const today=new Date().toISOString().slice(0,10); if (lastCompleted!==today) { setStreak(s=>s+1); setLastCompleted(today); } } };
